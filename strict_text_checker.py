...
with open("c:/PRUANED/build_estatutos.py", "r", encoding="utf-8") as f:
    code = f.read()

match = re.search(r'html_content = """(.*?)"""', code, re.DOTALL)
if not match:
    print("Could not find html_content in build_estatutos.py")
    exit(1)
...
for p in ocr_paragraphs:
    ...
    if clean_p not in pure_text:
        missing.append(clean_p)

if not missing:
    print("PERFECT 100% MATCH! All OCR text paragraphs exist exactly in HTML!")
else:
    print(f"FOUND {len(missing)} MISSING / MISMATCHED PARAGRAPHS:")
    for m in missing:
        print("  MISSING:", m[:80] + "...")
