...
import { Calendar, Download, ExternalLink, FileCheck, FileText, Search } from 'lucide-react';
import { useAuth } from '../context/AuthContext';

export const DocumentsSection = () => {
  const { documentsList = [], docCategories = [] } = useAuth();
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
        <header className="border-b border-slate-200 pb-6">
          <div className="inline-flex items-center gap-2 rounded-full bg-blue-100 px-3 py-1 text-xs font-bold uppercase tracking-wider text-blue-900">
            <FileCheck className="h-3.5 w-3.5" aria-hidden="true" /> Repositorio documental transparente
          </div>
          <h2 className="mt-3 font-['Outfit'] text-3xl font-extrabold sm:text-4xl">Documentos públicos y normativa gremial</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Actas, estatutos, reglamentos, protocolos, informes y otros documentos institucionales publicados por PRUANED A.G.</p>
        </header>
        ...
        {documents.length ? <div className="grid gap-6 md:grid-cols-2">
          {documents.map((document) => <article key={document.id} className="flex min-w-0 flex-col justify-between space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
              ...
              {document.url ? <a href={document.url} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-blue-900 px-4 py-2 text-xs font-bold text-white shadow transition hover:scale-[1.02] hover:bg-blue-800"><Download className="h-3.5 w-3.5 text-emerald-300" aria-hidden="true" />Abrir documento <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" /></a> : <span className="text-xs font-semibold text-slate-400">Archivo no disponible</span>}
            </div>
          </article>)}
